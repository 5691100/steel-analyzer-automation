...
| File | Purpose |
|------|---------|
| `agent-core/scripts/steel-drive.mjs` (837 lines) | Google Drive list/download/upload with MD5 verification and atomic manifests. Recursive subfolder download (`downloadRecursive`). Uses User OAuth only — no Service Account fallback. |
...
| `agent-core/src/telegram-bot.mjs` (351 lines) | grammy Telegram bot. Drive-link intake (paste URL → auto run_id), 5-gate approval flow (G1-G5) with Approve/Reject/Defer/Clarify/Open-chat buttons, Open-chat Q&A mode, improved /status with ledger state, `/inbox` email batch on-demand (single-concurrency guard). Security: `TELEGRAM_CHAT_ID` gate. |
| `agent-core/src/pipeline-runner.mjs` (402 lines) | `runPipeline(runId, folderId, notifyFn)` — preprocessing: unzip .zip, IFC→txt (ifcopenshell), xlsx→txt (openpyxl), DWG→file index; pipeline: G1→G2→G3→Phase:self-checklist→Phase:dashboard→Phase:GEPA→G5, Codex/GEPA flow via gepa-via-codex.mjs (non-blocking) |
...
| `agent-core/src/llm-dispatcher.mjs` (448 lines) | `dispatchClaudeAnalysis` — calls `claude -p -` via stdin with 160K char budget (file priority: BoM>RFQ>drawings>other), 50MB maxBuffer, parses JSON, generates workbooks, writes `analysis.json`. `dispatchAntigravityQA` — Claude QA self-check. `dispatchCodexReview` — uses `callCodex` (Codex→Claude fallback), writes `codex-review.json`. `writeGepaRegister` — writes `gepa-register.json`. `dispatchOpenChatQuestion`. Exports `extractJsonFromText`. |
